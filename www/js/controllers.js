angular.module('app.controllers', [])
  ...
  .controller('topicsCtrl', function($scope,$rootScope,$ionicSideMenuDelegate,
                                     ...
                                     $ionicHistory,$firebaseArray,sharedUtils, $timeout) {
    ...
    $scope.MQTTconnect=function() {
      ...
      console.log("START",mqttData);
      client = new Paho.MQTT.Client(
        "broker.mqttdashboard.com",
        8000,
        "clientId-mibabFssp2v1V"  //Client Id
      );
      // client = new Paho.MQTT.Client(
      //   mqttData.url,
    ...
    function onConnect() {
      ...
      // Initialize valued for widgets and subscribe to their topics
      $timeout(function(){
        angular.forEach($scope.topics,function(val,i){
          client.subscribe(val.topic);
          
          // Register watch on switch-type topics
          // We detect mutation on their values, and publish their state to the topic
          if( val.type=='switch' ){
            $scope.$watch(function() {
              return val.value;
            }, function(newval,old) {
              console.log('watch fire',newval,old)
              if( newval !== old ){
                console.log('watch pass',newval,old)
                $scope.toggleSwitchState(val);
              }
            });
          }
    ...
    function onMessageArrived(message) {
      console.log('arrived')
      angular.forEach( $scope.topics , function(val,i){
          if(val.topic == message.destinationName && val.type == 'thermostat' ){
            $timeout(function(){
              val.value = Number(message.payloadString);
            });
            val.value = message.payloadString;
          }
        });
    }

    // Toggle state of switch
    $scope.toggleSwitchState=function( item ){
      console.log('send',item.value);
      if( item.type == 'switch' ){
        var msg = item.value ? 'On' : 'Off';
        message = new Paho.MQTT.Message(msg); // -1 => Notify
    ...
    $scope.itemManipulation = function(edit_val) {  // Takes care of item add and edit ie Item Manipulator
      ...
      if(edit_val!=null) {
        ...
        $scope.data = { type:'switch' };    // For adding new address
        title="Add Topic";
        sub_title="Add a new topic";
      }
      // An elaborate, custom popup
      var connectionPopup = $ionicPopup.show({
        templateUrl: "templates/add-new-topic.html",
        title: title,
        subTitle: sub_title,
        scope: $scope,
        buttons: [
  ...
  .controller('graphCtrl', function($scope,$rootScope,fireBaseData,
    ...
    $scope.MQTTconnect=function() {
      ...
      client = new Paho.MQTT.Client(
        "broker.mqttdashboard.com",
        8000,
        "clientId-mibbFps2v1V"  //Client Id
      );
      // client = new Paho.MQTT.Client(
      //   mqttData.url,
      //   Number(mqttData.port),
